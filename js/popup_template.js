...
function templateTicketEditing(column, ticket) {    
    return /*html*/`<form class="add-task-form-style-board-wrapper column flex" onsubmit="editCategory(${column}, ${ticket}), saveChanges(${column}, ${ticket}); return false">      
                        ...
                        <div onclick="deleteTicket(${column}, ${ticket})" class="create-task-btn-container-edit create-btn-trash add-task-btn trashImage">

                        <img class="deleteIcon" src="./assets/img/deleteIcon.png">
                        </div>
                    </form>`;
}


function deleteTicket(column, ticket) {
...
function templateAddTaskFormInBoardCategory(column, ticket) {
    return `<div class="add-task-column-left-child flex column">
    <span class=>Category</span>
    <div class="margin-bottom-24">
        <div class="dropdown-container">
            <div class="dropdown" id="category-dropdown-field" onclick="changeVisibility('category-dropdown')">
                <div id="dropdown-text-category" class="flex category-dropdown-field-text">Select task category</div>
                <img src="./assets/img/vector_2.png">
            </div>
            <div class="dropdown-content d-none" id="category-dropdown"></div>
            <div class="w-100 input-container d-none" id="new-category-input-ctn">  <!--displays when new category gets clicked-->
                <input type="text" id="new-category-input" class="w-100 subtask-input" maxlength="50">
                <div class="flex">
                    <img src="./assets/img/add_task_cancel.png" class="icon-subtask" onclick="changeVisibilityNewCategory(), removeClassFromSelectedColor()">
                    |
                    <img src="./assets/img/add_task_check.png" class="icon-subtask" onclick="addNewCategory()">
                </div>
            </div>
        </div>
        <div id="category-color-selection-ctn" class="flex d-none"></div>
    </div>
</div>`;
}
...
async function editCategory(column, ticket) {
    if(saveIndexForEditingCategory != undefined) {
            boardColumns[column][ticket]['category'] = category[saveIndexForEditingCategory];
...
function templateAddTaskFormInBoardDuedatePrio(column, ticket) {
    return `<div class="w-100 flex column">
                <span>Due Date</span>
                <input type="date" id="due-date" min="" value="${boardColumns[column][ticket]['due-date']}" class="add-task-input margin-bottom-24" min="2022-10-01" max="2030-12-31">
            </div>
            <div class="w-100 flex column">
                <span>Prio</span>
                <div class="add-task-prio-container margin-bottom-24" id="prio-container"></div>
                <div class="add-task-prio-container-requirement " id="prio-container-requirement">Please selct a prority for your task</div>
 ...
 */
function templateNewContactPopupFormBtns(index) {
    return /*html*/`<div class="contacts-new-contact-popup-form-btns flex" id="contacts-new-contact-popup-form-btns">
                ...
                <div onclick="deleteContact(${index})" class="create-task-btn-container-edit create-btn-trash add-task-btn trashImage">
                <img class="deleteIcon" src="./assets/img/deleteIcon.png">
                </div>
...
async function deleteContact(index) {
    ...
    renderContactsList();
    closeContactsNewContactPopup();
    document.getElementById('contacts-info-popup-container').classList.remove('contact-info-popup-visible');
    await addContact();
}
