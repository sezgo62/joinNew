...
function greet() {
    ...
    greetUserOrGuestuser();
    greetDayTime(hour);
    
}

function greetUserOrGuestuser() {
    if(guestUser) {
...
/** That function gets the current user name, if a user is logged in with a password.
 * It is necessary for greeting the user on the summary.html page. */
...
function renderPriorityContainer(j) {
    if(urgent >= 1) {
        renderPrioContainerForUrgent();
    } else if(mid >= low && mid >= 0 && low >= 0) {
        renderPrioContainerForMid();
        } else if(low > mid) {
            renderPrioContainerForLow();
    }
...
function renderPrioContainerForUrgent() {
...
function renderPrioContainerForMid() {
...
function renderPrioContainerForLow() {
